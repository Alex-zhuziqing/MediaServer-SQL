...
#         return redirect(url_for('login'))
    ...
    genre_info = database.get_genre_type(genre_id)
    genre_type = genre_info['md_type_name']
    genre_name = genre_info['md_value']
    if genre_type == 'song genre':
        genre_items = database.get_genre_songs(genre_id)
    elif genre_type == 'film genre':
        genre_items = database.get_genre_movies_and_shows(genre_id)
    elif genre_type == 'podcast genre':
        genre_items = database.get_genre_podcasts(genre_id)
...
def add_song():
    ...
    allartists = database.get_allartists()
    allalbums = database.get_allalbums()
    allgenres = database.get_allsonggenres()
    ...
    if(request.method == 'POST'):
        ...
        newdict['albumid'] = request.form['albumid']
        print("We have a value: ",newdict['albumid'])
        ...
        #forward to the database to manage insert
        songs = database.add_song_to_db(newdict['storage_location'],newdict['description'],newdict['song_title'],newdict['song_length'],newdict['song_genre'], newdict['artist_id'], newdict['albumid'])

        max_song_id = database.get_last_song()[0]['song_id']
        print("max_song_id is " + str(max_song_id))
