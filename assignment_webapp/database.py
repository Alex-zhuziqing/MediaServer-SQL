...
def get_alltvshows():
    ...
    try:
        ...
        #############################################################################
        sql = """SELECT t.tvshow_id, s.tvshow_title, COUNT(tvshow_id)
                  FROM mediaserver.tvepisode as t natural join mediaserver.tvshow as s
                  GROUP BY t.tvshow_id, s.tvshow_title
                  ORDER BY t.tvshow_id, s.tvshow_title;
        """
...
def get_song(song_id):
    ...
    try:
        ...
        #############################################################################
        sql = """SELECT song_title, length, array_to_string(array_agg(artist_name),',') as artists
                 FROM mediaserver.Song 
                 RIGHT OUTER JOIN mediaserver.Song_Artists USING (song_id)
...
def get_song_metadata(song_id):
    ...
    try:
        ...
        sql = """select * from
(select song_id, md_type_id, md_id, md_value, md_type_name
	from mediaserver.song s
	left outer join (mediaserver.mediaitem
	natural join mediaserver.mediaitemmetadata
	natural join mediaserver.metadata
	natural join mediaserver.metadatatype) mimd
	on (s.song_id=mimd.media_id)
union
select song_id, md_type_id, md_id, md_value, md_type_name
	from mediaserver.album_songs a
...
def get_podcast(podcast_id):
    ...
    try:
        ...
        sql = """
        SELECT *
        FROM mediaserver.podcast natural join mediaserver.podcastmetadata natural join mediaserver.metadata natural join mediaserver.metadatatype
        WHERE podcast_id = %s;
        """
...
def get_all_podcasteps_for_podcast(podcast_id):
    ...
    try:
        ...
        FROM mediaserver.mediaitem natural join mediaserver.podcast natural join mediaserver.podcastepisode
        WHERE podcast_id = %s
...
def get_album_songs(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT song_id, song_title, string_agg(artist_name, ', ') as artitsts
        FROM (mediaserver.song natural join mediaserver.song_artists) R inner join mediaserver.artist S on (S.artist_id = R.performing_artist_id) NATURAL JOIN mediaserver.album_songs NATURAL JOIN mediaserver.album
        WHERE album_id = %s
        GROUP BY song_id
        """
...
def get_album_genres(album_id):
    ...
    try:
        ...
        #############################################################################
        sql = """
        SELECT string_agg(DISTINCT md_value, ', ' ORDER by md_value) as genres
        FROM mediaserver.song S inner join mediaserver.audiomedia AM on (S.song_id=AM.media_id) natural join mediaserver.mediaitemmetadata natural join mediaserver.metadata natural join mediaserver.album_songs natural join mediaserver.album
        WHERE album_id = %s
        GROUP BY album_id
        """
...
def get_genre_type(genre_id):
    """
    New function, determines genre type
    """
    conn = database_connect()
    if(conn is None):
        return None
...
##
##
# This is what the hardcoded data shows:
...
# It's a serial, so song genre is md_type_id 1, film genre is md_type_id 2, etc.
##
##
...
def get_genre_songs(genre_id):
    ...
    try:
        ...
        sql = """SELECT DISTINCT(song_id) AS item_id, song_title AS item_title, 'Song' AS item_type, md_id, md_value FROM mediaserver.MetaData NATURAL JOIN mediaserver.MediaItemMetaData JOIN mediaserver.Song ON (media_id=song_id)
...
def get_genre_podcasts(genre_id):
    ...
    try:
        ...
        ## Note that the hardcoded data does not contain any podcast genre information
        
        sql = """SELECT DISTINCT(podcast_id) AS item_id, podcast_title AS item_title, 'Podcast' AS item_type FROM mediaserver.MetaData NATURAL JOIN mediaserver.PodcastMetaData NATURAL JOIN mediaserver.Podcast
...
def get_genre_movies_and_shows(genre_id):
    ...
    try:
        ...
        #### Need to add TV Shows
        
        sql = """SELECT item_id, item_title, item_type FROM
(SELECT DISTINCT(movie_id) AS item_id, movie_title AS item_title, 'Movie' AS item_type, md_id FROM mediaserver.MetaData NATURAL JOIN mediaserver.MediaItemMetaData JOIN mediaserver.Movie ON (media_id=movie_id)
        WHERE md_type_id='2'
 UNION
 SELECT DISTINCT(tvshow_id) AS item_id, tvshow_title AS item_title, 'TV Show' AS item_type, md_id FROM mediaserver.MetaData NATURAL JOIN mediaserver.TVShowMetaData NATURAL JOIN mediaserver.TVShow
        WHERE md_type_id='2') AS sub
 WHERE sub.md_id=%s
 ORDER BY sub.item_id;
        """
...
def get_tvshow(tvshow_id):
    ...
    try:
        ...
        #############################################################################
        sql = """Select *
            From mediaserver.tvshow natural join mediaserver.tvshowmetadata natural join mediaserver.metadata natural join mediaserver.metadatatype
            Where tvshow_id = %s;
        """
...
def get_all_tvshoweps_for_tvshow(tvshow_id):
    ...
    try:
        ...
        #############################################################################
        sql = """Select tvshow_id, tvshow_episode_title, season, episode, air_date, media_id
        FROM mediaserver.mediaitem natural join mediaserver.tvshow natural join mediaserver.tvepisode
            WHERE tvshow_id = %s
...
def find_matchingmovies(searchterm):
    ...
    try:
        ...
        #############################################################################
        sql = """
            select m.movie_id,m.movie_title,m.release_year, count(md.md_id)
            from mediaserver.movie m left outer join mediaserver.mediaitemmetadata md on (m.movie_id = md.media_id)
            group by (m.movie_id)
            having lower(m.movie_title) ~ lower(%s);
        """
...
#   Add a new Song
#####################################################
def add_song_to_db(storage_location, description, title, songlength, genre, artist_id, albumid):
    """
    ...
    try:
        ...
        cur.execute(sql,(storage_location, description, title, songlength, genre, artist_id, albumid))
        conn.commit()                   # Commit the transaction
        r = cur.fetchone()
...
def get_allsonggenres():
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """
            select distinct md_value as genre 
            from mediaserver.metadatatype join mediaserver.metadata using (md_type_id)
